...
export default {
  ...
  beforeMount() {
    // if (Vue.isLoggedIn(( process.env.VUE_APP_BACKEND_URL || "" )+"/api/token/refresh/")) {
  ...
  methods: {
    ...
    onSuccess(googleUser) {
      ...
      Vue.axios.get(( process.env.VUE_APP_BACKEND_URL || "" )+"/api/check-linked-users/", config).then((res) => {
        if (res.status === 200) {
